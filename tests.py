...
def test_get_receipts__list_with_multiple_ingredients(cocktails_obj):
    ...
    assert result[0][0].components == ['Ананасовый сок, охлажденный - 3/4 стакана', 'ром - 1/2 стакана',
                                       'Сливки или кокосовое молоко - 1/2 стакана']
...
def test_get_receipts__list_with_another_multiple_ingredient(cocktails_obj):
    ...
    assert result[0][0].receipt == 'Влейте джин в стакан, наполовину заполненный колотым льдом. Добавьте Кюрасао, ' \
                                   'влейте тоник и перемешайте. Украсьте долькой лимона и веточкой мяты.'
    assert result[0][0].ingredients == {'ликер', 'джин', 'тоник'}
    assert result[0][0].components == ['Джин – 50 мл (3 ст. л.)', 'Ликер «Кюрасао» голубой – 12 мл (2 ч. л.)',
                                       'Тоник – 100 мл']
